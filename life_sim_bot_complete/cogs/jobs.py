...
class JobDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Energy cost shown instead of cooldown time
        energy_cost = job_data.get('energy_cost', 20)
        
        embed.add_field(name="💰 Current Salary", value=f"${salary}", inline=True)
        embed.add_field(name="⚡ Energy Cost", value=f"{energy_cost}⚡", inline=True)
...
class Jobs(commands.Cog):
    def __init__(self, bot):
        ...
        # Time-based work_cooldowns dictionary removed
        try:
            with open("assets/jobs.json") as f:
                self.jobs = json.load(f)
        except:
            self.jobs = [{"name": "laborer", "salary": 30, "req_rep": 0, "energy_cost": 20, "promotions": []}]
    ...
    async def work(self, ctx):
        ...
        current_energy = stats.get("energy", 0)
        ...
        # Determine energy cost (defaults to 20 if not in your jobs.json)
        energy_cost = job_data.get("energy_cost", 20) 

        # Check if user has enough energy
        if current_energy < energy_cost:
            return await ctx.send(f"❌ You are too exhausted to work! (Need **{energy_cost}⚡**, you have **{current_energy}⚡**)\n💡 Use `/daily` to restore your energy.")

        # Calculate Salary (Promotion system untouched)
        salary = job_data["salary"] + (promo_level * 20)
        ...
        if success:
            # Deduct Energy and Add Money/Reputation
            await players.update_one(
                {"user_id": ctx.author.id},
                {
                    "$inc": {
                        "money": salary, 
                        "stats.reputation": 5,
                        "stats.energy": -energy_cost # Energy deduction happens here
                    }
                }
            )
            title = job_data['promotions'][promo_level-1] if promo_level > 0 else job_id.replace('_', ' ').title()
            
            await ctx.send(f"✅ **{title}** Shift Complete!\n💰 Earned: **${salary}**\n⚡ Energy Used: **{energy_cost}** (Remaining: {current_energy - energy_cost}⚡)")
        else:
            # Minor energy penalty for the effort even on failure
            await players.update_one(
                {"user_id": ctx.author.id},
                {"$inc": {"stats.energy": -5}}
            )
            await ctx.send("❌ Task failed! You lost 5 energy trying.")
